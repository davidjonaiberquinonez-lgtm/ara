 ...
 */
final class ContadorProductosTool implements AgentToolInterface
{
    private const QUERY = 'SELECT COUNT(*) AS total_productos FROM ambart';

    public function getName(): string
    {
        return 'contar_productos';
    }

    public function getDescription(): string
    {
        return 'cuenta el número total de productos en la base de datos y se usa para obtener la cantidad de productos registrados.';
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO(
            ...
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'cuenta el número total de productos en la base de datos y se usa para obtener la cantidad de productos registrados.');
            if ($total > 0) {
