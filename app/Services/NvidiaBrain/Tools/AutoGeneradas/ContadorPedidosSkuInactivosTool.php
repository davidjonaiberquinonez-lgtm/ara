 ...
 */
final class ContadorPedidosSkuInactivosTool implements AgentToolInterface
{
    private const QUERY = 'SELECT COUNT(DISTINCT r.co_art) AS sku_distintos_7dias
FROM reng_fac r
INNER JOIN factura f ON r.fact_num = f.fact_num
INNER JOIN clientes c ON f.co_cli = c.co_cli
WHERE c.inactivo = :inactivo AND f.anulada = :anulada
  AND f.fec_emis >= DATEADD(day, -7, (SELECT MAX(fec_emis) FROM factura WHERE anulada = :anulada_2))';

    public function getName(): string
    {
        return 'contar_pedidos_sku_inactivos';
    }

    public function getDescription(): string
    {
        return 'cuenta la cantidad de SKUs distintos que han sido pedidos por clientes inactivos en los últimos 7 días hábiles';
    }
    ...
    public function getParameters(): array
    {
        return [
            ...
            'properties' => [
                ...
                'anulada_2' => ['type' => 'number', 'description' => 'Valor para el filtro anulada.'],
            ],
            'required' => ['inactivo', 'anulada', 'anulada_2'],
        ];
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO(
            ...
            $stmt->bindValue(':anulada_2', (float) ($arguments['anulada_2'] ?? 0));
            $stmt->execute();
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'cuenta la cantidad de SKUs distintos que han sido pedidos por clientes inactivos en los últimos 7 días hábiles');
            if ($total > 0) {
