 ...
 */
final class NotaEntregaFacturaTool implements AgentToolInterface
{
    private const QUERY = 'SELECT TOP 20 fact_num, num_doc, tipo_doc, reng_num FROM reng_fac WHERE fact_num = :fact_num';

    public function getName(): string
    {
        return 'obtener_nota_entrega';
    }

    public function getDescription(): string
    {
        return 'Obtiene la nota de entrega vinculada a una factura específica y debe usarse cuando se necesita consultar los documentos asociados a un número de factura dado.';
    }
    ...
    public function getParameters(): array
    {
        return [
            'type' => 'object',
            'properties' => [
                'fact_num' => ['type' => 'number', 'description' => 'Valor para el filtro fact_num.'],
            ],
            'required' => ['fact_num'],
        ];
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO(
            ...
            $stmt->bindValue(':fact_num', (float) ($arguments['fact_num'] ?? 0));
            $stmt->execute();
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'Obtiene la nota de entrega vinculada a una factura específica y debe usarse cuando se necesita consultar los documentos asociados a un número de factura dado.');
            if ($total > 0) {
