 ...
 */
final class ClienteSinDescuentoTool implements AgentToolInterface
{
    private const QUERY = 'SELECT TOP 1 co_cli, cli_des, desc_glob, desc_ppago, inactivo FROM clientes WHERE (desc_glob = :desc_glob OR desc_glob IS NULL) AND inactivo = :inactivo';

    public function getName(): string
    {
        return 'buscar_cliente_sin_descuento';
    }

    public function getDescription(): string
    {
        return 'Obtiene un cliente activo sin descuento global para usarlo en consultas de clientes sin beneficios.';
    }
    ...
    public function getParameters(): array
    {
        return [
            'type' => 'object',
            'properties' => [
                'desc_glob' => ['type' => 'number', 'description' => 'Valor para el filtro desc_glob.'],
                'inactivo' => ['type' => 'number', 'description' => 'Valor para el filtro inactivo.'],
            ],
            'required' => ['desc_glob', 'inactivo'],
        ];
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO(
            ...
            $stmt->bindValue(':desc_glob', (float) ($arguments['desc_glob'] ?? 0));
            $stmt->bindValue(':inactivo', (float) ($arguments['inactivo'] ?? 0));
            $stmt->execute();
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'Obtiene un cliente activo sin descuento global para usarlo en consultas de clientes sin beneficios.');
            if ($total > 0) {
