 ...
 */
final class NotaFacturaTool implements AgentToolInterface
{
    private const QUERY = 'SELECT fact_num FROM reng_fac WHERE num_doc = :num_doc';

    public function getName(): string
    {
        return 'buscar_factura_por_nota';
    }

    public function getDescription(): string
    {
        return 'Busca la factura asociada a una nota específica en la base de datos.';
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO(
            ...
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'Busca la factura asociada a una nota específica en la base de datos.');
            if ($total > 0) {
