 ...
 */
final class NotaEntrega23Tool implements AgentToolInterface
{
    private const QUERY = 'SELECT numero, nombre FROM usuarios WHERE numero IN (9, 23, 3)';

    public function getName(): string
    {
        return 'obtener_nota_entrega_2_3';
    }

    public function getDescription(): string
    {
        return 'Obtiene la nota de entrega asociada a una factura específica.';
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO(
            ...
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'Obtiene la nota de entrega asociada a una factura específica.');
            if ($total > 0) {
