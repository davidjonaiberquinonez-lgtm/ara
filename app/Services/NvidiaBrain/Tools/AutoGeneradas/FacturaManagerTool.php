 ...
 */
final class FacturaManagerTool implements AgentToolInterface
{
    private const QUERY = 'SELECT fact_num, co_cli, saldo, fec_emis, fec_venc, status, anulada FROM factura WHERE co_cli = :co_cli AND saldo > 0';

    public function getName(): string
    {
        return 'obtener_facturas_pendientes_2';
    }

    public function getDescription(): string
    {
        return 'Obtiene las facturas pendientes para un cliente específico identificado por su código, mostrando número de factura, saldo, fechas de emisión y vencimiento, estado y si está anulada.';
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO(
            ...
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'Obtiene las facturas pendientes para un cliente específico identificado por su código, mostrando número de factura, saldo, fechas de emisión y vencimiento, estado y si está anulada.');
            if ($total > 0) {
