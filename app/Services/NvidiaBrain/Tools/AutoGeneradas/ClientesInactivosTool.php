 ...
 */
final class ClientesInactivosTool implements AgentToolInterface
{
    private const QUERY = 'SELECT TOP 5 co_cli, cli_des, inactivo FROM clientes WHERE inactivo = :inactivo';

    public function getName(): string
    {
        return 'obtener_clientes_inactivos';
    }

    public function getDescription(): string
    {
        return 'Obtiene los primeros 5 clientes inactivos de la tabla clientes.';
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO(
            ...
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'Obtiene los primeros 5 clientes inactivos de la tabla clientes.');
            if ($total > 0) {
