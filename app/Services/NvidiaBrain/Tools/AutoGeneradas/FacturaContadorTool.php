 ...
 */
final class FacturaContadorTool implements AgentToolInterface
{
    private const QUERY = 'SELECT COUNT(*) AS cantidad_facturas_pendientes FROM facturas WHERE co_cli = :co_cli AND saldo > 0';

    public function getName(): string
    {
        return 'contar_facturas_pendientes_2';
    }

    public function getDescription(): string
    {
        return 'Devuelve la cantidad de facturas con saldo pendiente para un cliente específico, útil para verificar su estado de cuenta o deuda.';
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO('sqlite:C:\\ARA_PROYECT\\ara\\ARA_Brain\\data\\proyecto_ara.db', null, null, [
            ...
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'Devuelve la cantidad de facturas con saldo pendiente para un cliente específico, útil para verificar su estado de cuenta o deuda.');
            if ($total > 0) {
