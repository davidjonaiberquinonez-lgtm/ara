 ...
 */
final class UsuarioDAOTool implements AgentToolInterface
{
    private const QUERY = 'SELECT * FROM usuarios WHERE numero = :numero;';

    public function getName(): string
    {
        return 'obtener_nota_usuario';
    }

    public function getDescription(): string
    {
        return 'obtiene la información del usuario por su número único y se usa cuando necesitas consultar los detalles de un usuario específico.';
    }
    ...
    public function getParameters(): array
    {
        return [
            'type' => 'object',
            'properties' => [
                'numero' => ['type' => 'number', 'description' => 'Valor para el filtro numero.'],
            ],
            'required' => ['numero'],
        ];
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO(
            ...
            $stmt->bindValue(':numero', (float) ($arguments['numero'] ?? 0));
            $stmt->execute();
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'obtiene la información del usuario por su número único y se usa cuando necesitas consultar los detalles de un usuario específico.');
            if ($total > 0) {
