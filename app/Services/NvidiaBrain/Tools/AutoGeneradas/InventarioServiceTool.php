 ...
 */
final class InventarioServiceTool implements AgentToolInterface
{
    private const QUERY = 'SELECT co_art, art_des, stock_act, stock_com, stock_lle, prec_vta1, prec_vta2, ubicacion, co_prov FROM art WHERE UPPER(art_des) LIKE \'%EVIGAX%\' ORDER BY art_des';
    ...
    {
        return 'Busca artículos en el inventario por su descripción cuando necesitas localizar productos específicos en la base de datos.';
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO(
            ...
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'Busca artículos en el inventario por su descripción cuando necesitas localizar productos específicos en la base de datos.');
            if ($total > 0) {
