 ...
 */
final class ContadorNotasEmbaladas2Tool implements AgentToolInterface
{
    private const QUERY = 'SELECT COUNT(*) AS total, SUM(CASE WHEN verifi_emb = :verifi_emb THEN 1 ELSE 0 END) AS embaladas FROM gestion WHERE cd_barr LIKE :cd_barr AND DATE(hora) = \'2026-08-27\'';

    public function getName(): string
    {
        return 'contar_notas_embaladas_2_2';
    }

    public function getDescription(): string
    {
        return 'Cuenta las notas de una sede (prefijo 72) del día actual y cuántas están embaladas (verificadas).';
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO(
            ...
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'Cuenta las notas de una sede (prefijo 72) del día actual y cuántas están embaladas (verificadas).');
            if ($total > 0) {
