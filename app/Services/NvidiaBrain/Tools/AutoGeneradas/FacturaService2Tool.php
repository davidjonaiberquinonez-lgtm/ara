 ...
 */
final class FacturaService2Tool implements AgentToolInterface
{
    private const QUERY = 'SELECT co_cli FROM factura WHERE fact_num = :fact_num';

    public function getName(): string
    {
        return 'obtener_cliente_factura_2';
    }

    public function getDescription(): string
    {
        return 'obtiene el cliente al que pertenece una factura específica y se usa cuando se necesita identificar al cliente por medio del número de factura.';
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO(
            ...
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'obtiene el cliente al que pertenece una factura específica y se usa cuando se necesita identificar al cliente por medio del número de factura.');
            if ($total > 0) {
