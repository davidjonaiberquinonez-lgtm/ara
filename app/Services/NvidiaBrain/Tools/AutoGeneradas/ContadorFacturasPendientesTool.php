 ...
 */
final class ContadorFacturasPendientesTool implements AgentToolInterface
{
    private const QUERY = 'SELECT COUNT(*) AS cantidad_facturas_pendientes FROM facturas WHERE co_cli = :co_cli AND saldo > 0';

    public function getName(): string
    {
        return 'contar_facturas_pendientes';
    }

    public function getDescription(): string
    {
        return 'cuenta la cantidad de facturas pendientes para un cliente específico identificado por su código';
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO('sqlite:C:\\ARA_PROYECT\\ara\\ARA_Brain\\data\\proyecto_ara.db', null, null, [
            ...
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'cuenta la cantidad de facturas pendientes para un cliente específico identificado por su código');
            if ($total > 0) {
