 ...
 */
final class ContadorClientesTool implements AgentToolInterface
{
    private const QUERY = 'SELECT COUNT(*) AS total_clientes_inactivos FROM clientes WHERE inactivo = :inactivo;';

    public function getName(): string
    {
        return 'contar_clientes_inactivos';
    }

    public function getDescription(): string
    {
        return 'cuenta el número total de clientes con estado inactivo y se usa para obtener esa métrica específica.';
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO(
            ...
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'cuenta el número total de clientes con estado inactivo y se usa para obtener esa métrica específica.');
            if ($total > 0) {
