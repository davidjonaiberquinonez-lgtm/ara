 ...
 */
final class NotaEntrega2Tool implements AgentToolInterface
{
    private const QUERY = 'SELECT num_nota FROM rutagrama_notas WHERE factura_num = :factura_num';

    public function getName(): string
    {
        return 'obtener_nota_entrega_2';
    }

    public function getDescription(): string
    {
        return 'Obtiene la nota de entrega asociada a una factura específica cuando se necesita verificar el estado de entrega de una factura.';
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO('sqlite:C:\\ARA_PROYECT\\ara\\ARA_Brain\\data\\proyecto_ara.db', null, null, [
            ...
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'Obtiene la nota de entrega asociada a una factura específica cuando se necesita verificar el estado de entrega de una factura.');
            if ($total > 0) {
