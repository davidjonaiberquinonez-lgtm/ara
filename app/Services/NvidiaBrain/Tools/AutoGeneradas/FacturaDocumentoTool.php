 ...
 */
final class FacturaDocumentoTool implements AgentToolInterface
{
    private const QUERY = 'SELECT TOP 100 num_doc, tipo_doc, fact_num FROM reng_fac WHERE fact_num = :fact_num';
    ...
    {
        return 'Recupera la nota de entrega vinculada a una factura específica mediante su número.';
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO(
            ...
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'Recupera la nota de entrega vinculada a una factura específica mediante su número.');
            if ($total > 0) {
