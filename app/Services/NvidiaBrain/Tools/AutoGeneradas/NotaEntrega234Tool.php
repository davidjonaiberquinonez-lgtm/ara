 ...
 */
final class NotaEntrega234Tool implements AgentToolInterface
{
    private const QUERY = 'SELECT cd_barr, co_cli, cli_des, cant_items, num_prep, num_cheq, num_emb, hora, hora2, hora3 FROM gestion WHERE cant_items = :cant_items LIMIT 5';

    public function getName(): string
    {
        return 'buscar_nota_entrega_items_2_3_4';
    }

    public function getDescription(): string
    {
        return 'Busca notas de entrega que contengan un número específico de ítems, útil para filtrar pedidos por cantidad de productos.';
    }
    ...
    public function getParameters(): array
    {
        return [
            'type' => 'object',
            'properties' => [
                'cant_items' => ['type' => 'number', 'description' => 'Valor para el filtro cant_items.'],
            ],
            'required' => ['cant_items'],
        ];
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO(
            ...
            $stmt->bindValue(':cant_items', (float) ($arguments['cant_items'] ?? 0));
            $stmt->execute();
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'Busca notas de entrega que contengan un número específico de ítems, útil para filtrar pedidos por cantidad de productos.');
            if ($total > 0) {
