 ...
 */
final class FacturaServiceTool implements AgentToolInterface
{
    private const QUERY = 'SELECT TOP 5 fact_num, co_cli, fec_emis, fec_venc, tot_neto, saldo, status FROM factura WHERE co_cli LIKE :co_cli ORDER BY fec_emis DESC, fact_num DESC';

    public function getName(): string
    {
        return 'obtener_facturas_cliente';
    }

    public function getDescription(): string
    {
        return 'Obtiene las últimas 5 facturas de un cliente específico ordenadas por fecha de emisión y número de factura.';
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO(
            ...
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'Obtiene las últimas 5 facturas de un cliente específico ordenadas por fecha de emisión y número de factura.');
            if ($total > 0) {
