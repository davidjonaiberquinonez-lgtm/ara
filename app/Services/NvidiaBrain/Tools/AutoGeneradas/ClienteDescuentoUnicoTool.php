 ...
 */
final class ClienteDescuentoUnicoTool implements AgentToolInterface
{
    private const QUERY = 'SELECT co_cli, cli_des, porc_esp, desc_glob, desc_ppago FROM clientes WHERE porc_esp = :porc_esp AND inactivo = :inactivo';

    public function getName(): string
    {
        return 'buscar_cliente_descuento_unico';
    }

    public function getDescription(): string
    {
        return 'Busca un cliente activo con descuento especial del 100% para identificar descuentos únicos.';
    }
    ...
    public function getParameters(): array
    {
        return [
            'type' => 'object',
            'properties' => [
                'porc_esp' => ['type' => 'number', 'description' => 'Valor para el filtro porc_esp.'],
                'inactivo' => ['type' => 'number', 'description' => 'Valor para el filtro inactivo.'],
            ],
            'required' => ['porc_esp', 'inactivo'],
        ];
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO(
            ...
            $stmt->bindValue(':porc_esp', (float) ($arguments['porc_esp'] ?? 0));
            $stmt->bindValue(':inactivo', (float) ($arguments['inactivo'] ?? 0));
            $stmt->execute();
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'Busca un cliente activo con descuento especial del 100% para identificar descuentos únicos.');
            if ($total > 0) {
