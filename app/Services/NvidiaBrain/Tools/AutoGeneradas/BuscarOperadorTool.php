 ...
 */
final class BuscarOperadorTool implements AgentToolInterface
{
    private const QUERY = 'SELECT id, numero, nombre, categoria, activo FROM usuarios WHERE id = :id OR numero = :numero';

    public function getName(): string
    {
        return 'buscar_operador';
    }

    public function getDescription(): string
    {
        return 'Busca un operador por su id o número y devuelve sus datos básicos.';
    }
    ...
    public function getParameters(): array
    {
        return [
            'type' => 'object',
            'properties' => [
                'id' => ['type' => 'number', 'description' => 'Valor para el filtro id.'],
                'numero' => ['type' => 'string', 'description' => 'Valor para el filtro numero.'],
            ],
            'required' => ['id', 'numero'],
        ];
    }
    ...
    public function execute(array $arguments, array $contexto): array
    {
        ...
        $pdo = new PDO(
            ...
            $stmt->bindValue(':id', (float) ($arguments['id'] ?? 0));
            $stmt->bindValue(':numero', (string) ($arguments['numero'] ?? ''));
            $stmt->execute();
            $filas = $stmt->fetchAll();
            $total = count($filas);
            $cb = CardBuilder::iniciar('🤖', 'Busca un operador por su id o número y devuelve sus datos básicos.');
            if ($total > 0) {
